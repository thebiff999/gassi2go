...
class EntryComponent extends PageMixin(LitElement) {
  static styles = [
    css`
      ${unsafeCSS(entryDetailsComponentCSS)}
    `
  ];
  ...
  render() {
    const template = html` ${this.renderNotification()}
    ${this.exists
      ? html`
          <div class="container">
            <div class="row">
              <div class="col-sm-12 col-lg-5 image">
                <i
                  id="mobile-button"
                  class="fas fa-arrow-circle-left fa-3x back-button hover-button mobile-button"
                  style="color: white"
              ...
              <div class="h-100 col-sm-12 col-lg-7">
                <div class="details">
                  <i
                    id="desktop-button"
                    class="fas fa-arrow-circle-left fa-lg back-button hover-button desktop-button"
                    style="color: blue"
                    @click="${this.navigateBack}"
                  ></i>
  ...
  async submit() {
    const requestBody = this.entry;
    requestBody.status = 'assigned';
    try {
      console.log('patch-request');
      console.log(requestBody);
      await httpClient.patch('/entries/id/' + this.entryId, requestBody);
      const notification = 'Du hast den Auftrag angenommen und kannst ihn unter Meine Aufträge finden';
