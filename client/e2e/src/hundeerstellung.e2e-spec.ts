...
describe('/user/dogs/new', () => {
    ...
    it('should sucessfully create a dog with the default-image', async() => {
        ...
        await page.fill('textarea', 'Lucky ist ein richtiger Frechdachs! Wenn er nicht an der Leine läuft, ist er sofort weg.');
        expect(await Promise.all([page.waitForResponse(response => response.status() === 201), await page.click('text=Hund anlegen')])).toBeTrue;
    });
    ...
    it('should show invalid-feedback when the additional informations are too short', async() => {
        ...
        await page.click('text=Hund anlegen');
        const error = await page.textContent('app-hundeerstellung .invalid-feedback:visible');
        expect(error).toBe('Die Zusätzlichen Informationen sind erforderlich und müssen zwischen 30 und 1000 Zeichen lang sein.');
    });
});